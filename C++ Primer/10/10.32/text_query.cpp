...
void text_query::build_map()
{
	...
	for(ix_line = 0; ix_line != lines_of_text.size(); ++ix_line)
	{
		...
		while(iss_line >> word)
		{
			...
			{
				word_map[word].push_back(ix_line);
			}
			else if(word_map[word].back() != ix_line)
			{
				word_map[word].push_back(ix_line);
			}
		}
...
std::vector<text_query::line_no> text_query::run_query(const std::string &word) const
{
	std::map<std::string, std::vector<text_query::line_no> >::const_iterator it = word_map.find(word);
	if(it == word_map.end())
	{
		return std::vector<text_query::line_no>();
	}
	else
	{
		return it -> second;
