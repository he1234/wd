/********************************************************************
  ...
  > Created Time: 2015年03月12日 星期四 23时25分20秒
 *******************************************************************/
...
int main()
{
	...
	const point ptC(1, 2);	//声明一个const对象（类变量）,该const意思是只能调用const成员函数和构造以及析构函数，
	//ptC.SetX(8);			//错误，ptC是const对象，只能调用const成员函数
	//ptC.SetY(9);			//错误，ptC是const对象，只能调用const成员函数
	ptC.~point();			//正确，const对象也能调用非const类型的析构函数，该const本意是只能调用const成员函数和构造以及析构函数，无关乎修改
