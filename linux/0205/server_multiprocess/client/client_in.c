...
int main(int argc, char* argv[])//shm_key sem_key
{
	...
	char msg[32] ="" ;
	int fd_r, fd_w ;
	sprintf(server_name, "%s/%s", SERVER_FIFO_PATH, SERVER_FIFO_NAME);
	int fd_server = open(server_name, O_WRONLY);

	memset(read_file, 0, 128);
	memset(write_file, 0, 128);

	sprintf(read_file, "%s/%d_r.fifo", SERVER_FIFO_PATH, getpid());
	sprintf(write_file, "%s/%d_w.fifo", SERVER_FIFO_PATH, getpid());
